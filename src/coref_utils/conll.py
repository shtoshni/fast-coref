...
def official_conll_eval(conll_scorer, gold_path, predicted_path, metric, official_stdout=False):
...
def evaluate_conll(conll_scorer, gold_path, predictions, subtoken_maps, prediction_path,
	...
	result = {
		metric: official_conll_eval(conll_scorer, gold_file.name, prediction_file.name,
		                            metric, official_stdout) for metric in ("muc", "bcub", "ceafe")}
	return result
