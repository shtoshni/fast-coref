...
class MentionProposalModule(nn.Module):
    def __init__(self, model_config, train_config):
        ...
        self._build_model(
            mention_params= model_config.mention_params,
    ...
    def device(self) -> torch.device:
        ...
        return next(self.span_width_embeddings.parameters()).device
    ...
    def get_pred_mentions(self, document, encoded_doc):
        num_words = encoded_doc.shape[0]

        filt_cand_starts, filt_cand_ends, flat_cand_mask = self.get_candidate_endpoints(encoded_doc, document)
        ...
        train_vars = {}
        if self.training:
            k = int(self.top_span_ratio * (document["subtoken_map"][-1] - document["subtoken_map"][0] + 1))
            topk_indices = torch.topk(mention_logits, k)[1]
            filt_gold_mentions = self.get_gold_mentions(document["clusters"], num_words, flat_cand_mask)
    ...
    def get_mention_embs(self, document):
        encoded_doc = self.doc_encoder(document)
        train_vars = None
        if not self.use_gold_ments:
            pred_starts, pred_ends, pred_scores, train_vars = self.get_pred_mentions(document, encoded_doc)
        else:
            pred_starts, pred_ends, pred_scores = self._get_gold_mentions(document)
    ...
    def _get_gold_mentions(self, document):
        '''Get gold mentions given the document.'''
        mentions = []
