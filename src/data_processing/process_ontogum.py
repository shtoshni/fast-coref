...
def parse_cluster_info(string):
    cur_cluster_str = ''
    cluster_start = False

    cluster_start_list, cluster_end_list, cluster_start_end_list = [], [], []

    for char in string:
        if char.isdigit():
            cur_cluster_str += char
        else:
            if char == '(':
                if cur_cluster_str != '':
                    cluster_start_list.append(int(cur_cluster_str))
                cluster_start = True
            elif char == ')':
                if cluster_start:
                    cluster_start_end_list.append(int(cur_cluster_str))
                else:
                    cluster_end_list.append(int(cur_cluster_str))
                cluster_start = False
    ...
    return cluster_start_list, cluster_end_list, cluster_start_end_list
        ...
        for seg_idx, segment in enumerate(self.segment_info):
            for i, tok_info in enumerate(segment):
                ...
                if coref != "_":
                    ...
                    cluster_start_list, cluster_end_list, cluster_start_end_list = parse_cluster_info(coref)
                    # print(coref, cluster_start_list, cluster_end_list, cluster_start_end_list)
                    for cluster_id in cluster_start_list:
                        self.coref_stacks[cluster_id].append(first_subtoken_index)

                    for cluster_id in cluster_end_list:
                        # self.coref_stacks[cluster_id].append(first_subtoken_index)
                        start = self.coref_stacks[cluster_id].pop()
                        self.clusters[cluster_id].append((start, last_subtoken_index))

                    for cluster_id in cluster_start_end_list:
...
def minimize_split(args):
    ...
    minimize_partition("test", args)


def consolidate_conll(args):
    """Concatenate all the CoNLL files into one file"""
    output_file = path.join(args.input_dir, 'test.conll')

    with open(output_file, 'w') as out_f:
        for gum_file in glob.glob(path.join(args.input_dir, "GUM*.conll")):
            with open(gum_file) as read_f:
                # Remove extension, GUM_ part from file
                filename = ((path.split(gum_file)[-1]).split('.')[0])[4:]
                first_line = True
                ...
                for line in read_f:
                    ...
                    if first_line:
                        first_line = False
                        output_line = "#begin document " + f"({filename}); part 0"
